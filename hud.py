...
class ArcReactor(QWidget):
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        for alpha in (18, 28, 42, 65):
            glow_radius = radius + 34 + pulse
        ...
        for offset, span, width in ((self.phase * 40, 105, 3), (-self.phase * 25, 55, 2), (20, 25, 4)):
            ...
            painter.setPen(pen)
            painter.drawArc(
                int(center.x() - radius - 11), int(center.y() - radius - 11),
                int((radius + 11) * 2), int((radius + 11) * 2),
                int((offset % 360) * 16), int(span * 16),
            )
...
class JarvisHUD(QMainWindow):
    def __init__(self):
        ...
        self.log_label.setStyleSheet("color:#8fe6ff; background:#061722; border:1px solid #12617b; border-radius:10px; padding:16px; font-family:Consolas; font-size:14px;")
        ...
        self.command_input.setStyleSheet("QLineEdit { background:#071521; color:#dffaff; border:1px solid #2abbdf; border-radius:8px; padding:10px; font-size:15px; }")
        self.execute_button = QPushButton("RUN")
        self.execute_button.setStyleSheet("QPushButton { background:#0c7ea3; color:white; border:none; border-radius:8px; padding:10px 18px; font-weight:700; }")
        ...
        central.setStyleSheet("QFrame#panel, QFrame#corePanel { background:#071521; border:1px solid #16738e; border-radius:14px; }")
        self.update_ui()
